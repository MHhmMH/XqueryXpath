...
public class XQueryExtendsVisitor extends XQueryBaseVisitor<ArrayList<Node>> {
	...
	private void helper(XQueryParser.XqFLWRContext ctx, int k, ArrayList<Node> result) {
	...
	public ArrayList<Node> visitFltEqual(XQueryParser.FltEqualContext ctx) {
		ArrayList<Node> temp = list;
		ArrayList<Node> rpList1 = visit(ctx.rp(0));
		list = temp;
		ArrayList<Node> rpList2 = visit(ctx.rp(1));
		list = temp;
		for (int i = 0; i < rpList1.size(); i++) {
			for (int j = 0; j < rpList2.size(); j++) {
				if (rpList1.get(i).isEqualNode(rpList2.get(j))) {
					return list;
				}
			}
		}
	...
	public ArrayList<Node> visitFltOr(XQueryParser.FltOrContext ctx) {
		...
		for (int i = 0; i < temp.size(); i++) {
			list = new ArrayList<Node>();
			list.add(temp.get(i));
			ArrayList<Node> filterList1 = visit(ctx.filter(0));
			list = new ArrayList<Node>();
			list.add(temp.get(i));
			ArrayList<Node> filterList2 = visit(ctx.filter(1));
			if (!filterList1.isEmpty() || !filterList2.isEmpty()) {
				result.add(temp.get(i));
			}
		}
		list = result;
	...
	public ArrayList<Node> visitFltRp(XQueryParser.FltRpContext ctx) {
		...
		for (int i = 0; i < temp.size(); i++) {
			list = new ArrayList<Node>();
			list.add(temp.get(i));
			ArrayList<Node> filterList = visit(ctx.rp());
			if (!filterList.isEmpty()) {
				result.add(temp.get(i));
			}
		}
	...
	public ArrayList<Node> visitFltIs(XQueryParser.FltIsContext ctx) {
		...
		for (int i = 0; i < rpList1.size(); i++) {
			for (int j = 0; j < rpList2.size(); j++) {
				if (rpList1.get(i) == rpList2.get(j)) {
					return list;
				}
			}
		}
